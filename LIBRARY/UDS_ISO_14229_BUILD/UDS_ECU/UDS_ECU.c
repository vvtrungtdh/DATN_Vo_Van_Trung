...
int processUDSService(Comm *comm, uint8_t request[7])
{
	...
	else if (comm->session_control == DEFAULT_SESSION && (service == ECU_RESET || service == CLEAR_DIAGNOSTIC_INFO || service == READ_DTC_INFO))
	{
		serviceHandlers[service](comm, request);
	}
	else if (comm->session_control == CUSTOM_SESSION && (service == ECU_RESET || service == CLEAR_DIAGNOSTIC_INFO || service == READ_DTC_INFO || service == READ_DATA_BY_IDENTIFIER || service == WRITE_DATA_BY_IDENTIFIER || service == REQUEST_DOWNLOAD))
	{
		serviceHandlers[service](comm, request);
	}
	else
...
{
	ISO iso;
	uint8_t ID[4] = {0xAB, 0xCD, 0xEF, 0xFF};
	...
	if (DID == BootSoftwareIdentificationDataIdentifier)
	{
		...
		iso.as.service22.dataRecord1 = ID[0];
		iso.as.service22.dataRecord2 = ID[1];
		iso.as.service22.dataRecord3 = ID[2];
		iso.as.service22.dataRecord4 = ID[3];
		SendCANData(TX_MAILBOX, TX_MSG_ID, iso.as.data.ptr, 7);
		return R_OK;
	}
...
int writeDataByIdentifierHandler(Comm *comm, uint8_t *request)
{
	...
	if (DID == BootSoftwareIdentificationDataIdentifier)
	{
		...
		ID[7] = 0x00;
		// function write ID
		writeDataToDFlash(DID_ADDRESS, sizeof(ID), ID);
...
int requestDownloadHandler(Comm *comm, uint8_t *request)
{
	...
	ret = FLASH_DRV_EraseSector(&flashSsdConfig, DID_ADDRESS, 4096);
	while (STATUS_SUCCESS != ret)
		;
	ret = FLASH_DRV_VerifySection(&flashSsdConfig, DID_ADDRESS, 4096 / 8, 1u);
	while (STATUS_SUCCESS != ret)
		;
	S32_SCB->AIRCR =
		S32_SCB_AIRCR_VECTKEY(0x5FA) | S32_SCB_AIRCR_SYSRESETREQ(1);
	return 0;
}
